...
fn recursive_search(ans: u64, val: u64, i:usize, arr: &Vec<usize>)-> Option<u64>{
    ...
    if let Some(result) = recursive_search(ans, concentrate(val, arr[i] as u64), i+1, arr){
        return Some(result);
    }
...
fn concentrate(x:u64, y:u64)->u64{
    let string_numbers = x.to_string() + &y.to_string();
    string_numbers.parse().unwrap()
}